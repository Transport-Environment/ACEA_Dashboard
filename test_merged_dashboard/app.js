...
// Top-level toggle between the landing (market overview) and country-eu (country breakdown)
// sections - same show/hide-a-container pattern as initQuarterlyChartToggle(), just for whole
// page sections instead of one chart box.
function initPageToggle() {
  const buttons = document.querySelectorAll(".page-toggle-btn");
  const pages = {
    landing: document.getElementById("landing"),
    country: document.getElementById("country-eu"),
  };
  if (!buttons.length) return;
  ...
  function showPage(pageKey) {
    ...
    if (pageKey === "country") {
      // The country page's Live charts initialize on window load regardless of visibility, so
      // a chart that was hidden until now can render at zero size; force a resize now that its
      // container has real dimensions (same fix as initQuarterlyChartToggle(), applied to all
      // of them since a whole section - not just one chart - was hidden).
      LIVE_CHARTS.forEach((chart) => {
        if (chart.visual) chart.visual.update(chart.options);
      });
    }
  }
  ...
  if (countryFromHash()) showPage("country");
}
